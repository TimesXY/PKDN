...
def roc_model(model, data_test):
    ...
    for i, (test_images, test_labels) in enumerate(data_test):
        ...
        # 获取输出
        model_predict_1 = model_out["Y1"]
        model_predict_2 = model_out["Y2"]

        # 获取综合预测值
        test_predicts = softmax(model_predict_1 + model_predict_2)
...
def confusion(model, data_test):
    ...
    for i, (test_images, test_labels) in enumerate(data_test):
        ...
        # 获取综合预测值
        test_predicts = (model_predict_1 + model_predict_2)
...
# 计算相关指标
def metrics_model(model, data_test):
    score_list = []  # 存储预测得分
    label_list = []  # 存储真实标签
...
def mix_up_data(x, y, alpha=1.0, use_cuda=True):
    """Compute the mix_up data. Return mixed inputs, pairs of targets, and lambda"""
    if alpha > 0.:
...
def create_id(model, loader_test):
    ...
    with torch.no_grad():
        for i, (test_images, test_labels, file_name) in enumerate(loader_test):
            ...
            # 预测输出
            test_predicts_1 = model_out["Y1"]
            test_predicts_2 = model_out["Y2"]
            test_predicts = test_predicts_1 + test_predicts_2
